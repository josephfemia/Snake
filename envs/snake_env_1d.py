...
class SnakeEnv1D(gym.Env):
    def __init__(self, grid_x=10, grid_y=10):
        ...
        self.SNAKE_LEN_GOAL = int(grid_x*grid_y*.10)
        max_distance = (int(np.sqrt(grid_x**2 + grid_y**2)) + 1)*10
        self.action_space = spaces.Discrete(4)
        self.observation_space = spaces.Box(low=-max_distance, high=max_distance, shape=(self.SNAKE_LEN_GOAL + 5, ), dtype=np.int32)
        self.grid_x = grid_x
        self.grid_y = grid_y
    ...
    def step(self, action):
        ...
        observation = [head_x, head_y, apple_delta_x, apple_delta_y, 
                       snake_length]
        return np.array(observation, dtype=np.int32), self.reward, self.done, info

    def reset(self):
        self.snake_position = [[self.grid_x // 2 * 10, self.grid_y // 2 * 10],
                               [(self.grid_x // 2 - 1) * 10, self.grid_y // 2 * 10],
                               [(self.grid_x // 2 - 2) * 10, self.grid_y // 2 * 10]]
